...
import AutorizationButton from "./components/AuthorizationButton"
...
function App() {
  ...
  useEffect(() => {
    axios
      .get("http://0.0.0.0:8888/lists?_expand=color&_embed=notes")
      .then(({ data }) => {
        setLists(data);
      });
    axios.get("http://0.0.0.0:8888/colors").then(({ data }) => {
      setColors(data);
    });
  }, []);
  ...
  const onAddNote = (listId, noteObj) => {
  ...
  const onEditnote = (listId, noteObj) => {
    const newnoteText = window.prompt("Имя заметки", noteObj.text);
    ...
    const newList = lists.map((list) => {
      if (list.id === listId) {
        list.notes = list.notes.map((note) => {
    ...
    setLists(newList);
    axios
      .patch("http://0.0.0.0:8888/notes/" + noteObj.id, {
        text: newnoteText,
      })
      .catch(() => {
        alert("Не удалось переименовать заметку");
      });
  };

  const onRemovenote = (listId, noteId) => {
      const newList = lists.map((item) => {
        if (item.id === listId) {
      ...
      setLists(newList);
      axios.delete("http://0.0.0.0:8888/notes/" + noteId).catch(() => {
  ...
  return (
    <div className="manager">
      ...
      <div>   
      <AutorizationButton onClick={()=>{console.log('authorization')}} />
      </div>
        <List
          onClickItem={(list) => {
            history.push(`/`);
